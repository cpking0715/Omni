# dump 所有含 messages 的 href, 观察会话链接真实格式
...
(() => {
  ...
  document.querySelectorAll('a[href*="messages"], a[href*="Messages"]').forEach(a => {
    const h = a.href || '';
    const t = (a.textContent || a.getAttribute('aria-label') || '').trim().replace(/\s+/g,' ').slice(0,40);
    hrefs.push(h.slice(0,140) + ' || ' + t);
  });
  if (hrefs.length === 0) {
    // 页面 URL + 是否有会话容器
    hrefs.push('pageUrl=' + location.href);
    const box = document.querySelector('[data-e2e="chat-list"]') || document.querySelector('[data-e2e="dm-list"]');
    hrefs.push('chatbox=' + (box ? box.className.slice(0,60) : 'none'));
  }
  return hrefs.slice(0, 40).join('\n');
})()
'@
