# 完整链路: 点击会话 → 输入 test → 点击发送 → CDP Network 域捕获 WS 帧 → DOM 验证
...
function Invoke-Cdp($ws, [int]$i, [string]$method, $params) {
    ...
    while ($true) {
        ...
        # 顺带收集 WS 帧事件 (binary payload 是 base64)
...
# 2. 验证会话打开
$uniqExpr = '(() => { const u = document.querySelector("[data-e2e=\"chat-uniqueid\"]"); return u ? u.textContent : "none"; })()'
$resp = Invoke-Cdp $ws 6 "Runtime.evaluate" @{ expression = $uniqExpr; returnByValue = $true }
$uniq = $resp.result.result.value
Write-Host "=== 会话: uniqueid=$uniq ==="
if ($uniq -eq "none" -or $uniq -eq "@") { Write-Host "会话未打开, 终止"; $ws.Dispose(); exit 1 }
# 3. 聚焦输入框 (JS focus + 坐标点击)
$focusExpr = @'
(() => {
  const ed = document.querySelector('[data-e2e="message-input-area"] [contenteditable="true"]') ||
            document.querySelector('[data-e2e="message-input-area"] div.DraftEditor-root [contenteditable="true"]') ||
...
(() => {
  ...
  out.lastMsgs = Array.from(msgs).slice(-4).map(el => (el.textContent||'').trim().replace(/\s+/g,' ').slice(0,80));
  return JSON.stringify(out);
})()
'@
$resp = Invoke-Cdp $ws 16 "Runtime.evaluate" @{ expression = $checkExpr; returnByValue = $true }
Write-Host "=== 发送后 ==="
Write-Host $resp.result.result.value
$shot = Invoke-Cdp $ws 17 "Page.captureScreenshot" @{ format = "png" }
[IO.File]::WriteAllBytes("d:\MyProjects\OmniMarket\ttdm\bin\m6\click_send.png", [Convert]::FromBase64String($shot.result.data))
Write-Host "=== WS 帧日志 ==="
