# rs_ui_enter2.ps1 — 最终发送验证: 干净状态 + insertText + Enter 键发送 + 全通道 hook
$ErrorActionPreference = 'Stop'
$outFile = "d:\MyProjects\OmniMarket\ttdm\bin\m6\out_uienter2.json"
...
$log = @()
$msg = "rs-enter2-" + [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()

# 1. 刷新页面 (清掉输入框累积文本, 恢复干净状态)
$null = Invoke-Cdp $ws 1 "Page.reload" @{ ignoreCache = $true }
$log += 'reload'
...
$hookExpr = @'
(() => {
  if (window.__e2Hooked) return 'already';
  window.__e2Hooked = true;
  window.__e2 = { http: [], wsOut: [], wsIn: [] };
  ...
  window.fetch = async (...args) => {
    ...
    if (url.indexOf('message/send') >= 0 || url.indexOf('im-api') >= 0) {
      try {
        ...
        window.__e2.http.push({ kind: 'fetch', url: url.slice(0, 120), status: r.status, code: m ? m[1] : '', body: s.slice(0, 150) });
      } catch (e) {}
    }
    return r;
  };
    ...
    this.addEventListener('load', () => {
      ...
      if (u.indexOf('message/send') >= 0 || u.indexOf('im-api') >= 0) {
        try {
          ...
          const m = s.match(/"status_code":(\d+)/);
          window.__e2.http.push({ kind: 'xhr', url: u.slice(0, 120), status: this.status, code: m ? m[1] : '', body: s.slice(0, 150) });
        } catch (e) {}
      }
    });
  ...
  const logWS = (dir, data) => {
    try {
      ...
      else if (data && data.data) s = typeof data.data === 'string' ? data.data : '[binary ' + (data.data.byteLength || '?') + 'B]';
      if (s && s.length < 8000) window.__e2[dir].push(s.slice(0, 3000));
    } catch (e) {}
  };
...
'@
$null = Invoke-Cdp $ws 2 "Runtime.evaluate" @{ expression = $hookExpr; returnByValue = $true }
$log += 'hook'
...
(async () => {
  ...
  while (Date.now() < deadline) {
    ...
    if (items.length > 0) {
      ...
      return 'opened: ' + (document.querySelector('[data-e2e="dm-new-chat-nickname"]')?.textContent.trim() || '?');
    }
...
(() => {
  ...
  out.btn = !!document.querySelector('svg[aria-label="Send"]');
...
'@
    ...
    # 6. Enter 键发送 (焦点在编辑器)
    $null = Invoke-Cdp $ws 9 "Input.dispatchKeyEvent" @{ type = "keyDown"; key = "Enter"; code = "Enter"; windowsVirtualKeyCode = 13; nativeVirtualKeyCode = 13 }
...
(() => {
  const out = {
    http: (window.__e2 && window.__e2.http) || [],
    wsOut: (window.__e2 && window.__e2.wsOut) || [],
    wsIn: (window.__e2 && window.__e2.wsIn) || [],
    chats: [], inputAfter: ''
  };
